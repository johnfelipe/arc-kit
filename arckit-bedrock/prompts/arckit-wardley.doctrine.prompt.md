...
Read `{{skills_wardley_mapping_references_doctrine}}`
...
- **First**, check if `{{templates_wardley_doctrine_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_wardley_doctrine_template}}` (default)
