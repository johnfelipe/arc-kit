...
1. **Read the template** (with user override support):
   - **First**, check if `{{templates_architecture_principles_template}}` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `{{templates_architecture_principles_template}}` (default)
...
8. **Write the output**:
   ...
   Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **PRIN** per-type checks pass. Fix any failures before proceeding.
