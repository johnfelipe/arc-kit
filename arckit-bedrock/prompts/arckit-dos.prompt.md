...
- **First**, check if `{{templates_dos_requirements_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_dos_requirements_template}}` (default)
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **DOS** per-type checks pass. Fix any failures before proceeding.
