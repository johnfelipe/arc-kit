...
4. **Read the template** (with user override support):
   - **First**, check if `{{templates_data_model_template}}` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `{{templates_data_model_template}}` (default)
...
6. **Load Mermaid Syntax Reference**:
   - Read `{{skills_mermaid_syntax_references_entityRelationshipDiagram}}` for official Mermaid ER diagram syntax — entity definitions, relationship types, cardinality notation, and attribute syntax.
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **DATA** per-type checks pass. Fix any failures before proceeding.
