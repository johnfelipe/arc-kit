...
**User Request**: {{input_architecture_docs}}
...
- **If no hook message** (hook unavailable or failed): fall back to manual sync and title extraction:
  1. Use **Glob** to list all `.md` files in `{{docs_guides}}` (and any subdirectories like `uk-government/`, `uk-mod/`, `roles/`)
...
- **First**, check if `{{templates_pages_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_pages_template}}` (default)
...
3. Replace the placeholder values **in memory** (string replacement) with actual repository details:
   ...
   - `'{{VERSION}}'` → the ArcKit version from the plugin's VERSION file (`{{VERSION}}`)
...
**Remember**: You MUST read and use `{{templates_pages_template}}` as the base for `docs/index.html`. The template is the source of truth for all HTML, CSS, and JavaScript. Only replace the `{{...}}` config placeholders with actual values.
