...
1. **A new document template** in `{{templates_custom_{name}_template}}` with `Template Origin: Community` banner
2. **A usage guide** in `{{guides_custom_{name}}}` with `Guide Origin: Community` banner
3. **Optionally**: A matching slash command in `.claude/commands/arckit.community.{name}.md`
4. **Optionally**: A shareable bundle in `{{community_{name}}}`
...
Extract the template type from `{{input_architecture_docs}}`. If blank or vague, ask the user what kind of document template they want to create.
...
- `{{templates_requirements_template}}` (canonical Document Control + structure)

**Read one more based on the user's category selection**:

- Governance: `{{templates_conformance_assessment_template}}`
- Technical: `{{templates_platform_design_template}}`
- Procurement: `{{templates_evaluation_criteria_template}}`
- Strategy: `{{templates_architecture_strategy_template}}`

### Step 4: Generate the Template

Create the template file at `{{templates_custom_{name}_template}}` using the Write tool.
...
Create a usage guide at `{{guides_custom_{name}}}` using the Write tool.
...
Output: `{{templates_custom_{name}_template}}`
...
- Edit `{{templates_custom_{name}_template}}` directly
...
1. Copy the bundle from `{{community_{name}}}` (if generated)
...
2. **Read the template**:
   - Read `{{templates_custom_{name}_template}}`
...
- `{{community_{name}_README}}` — Usage instructions, author info, description, and "Submit to ArcKit" section
- `{{community_{name}_{name}_template}}` — Copy of the template
- `{{community_{name}_{name}}}` — Copy of the usage guide
- `{{community_{name}_arckit_community_{name}}}` — Copy of the command (if generated)
...
- `{name}-template.md` -> `{{templates_custom}}`
- `{name}.md` -> `{{guides_custom}}`
...
2. Copy template to `{{templates}}` and `arckit-claude/templates/`
...
| File | Location |
|------|----------|
| Template | `{{templates_custom_{name}_template}}` |
| Usage Guide | `{{guides_custom_{name}}}` |
| Slash Command | `.claude/commands/arckit.community.{name}.md` (if selected) |
| Shareable Bundle | `{{community_{name}}}` (if selected) |
...
- Share the `{{community_{name}}}` bundle via Git
