...
2. **Read the template** (with user override support):
   - **First**, check if `{{templates_stakeholder_drivers_template}}` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `{{templates_stakeholder_drivers_template}}` (default)
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **STKE** per-type checks pass. Fix any failures before proceeding.
