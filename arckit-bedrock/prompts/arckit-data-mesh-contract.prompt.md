...
- **First**, check if `{{templates_data_mesh_contract_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_data_mesh_contract_template}}` (default)
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **DMC** per-type checks pass. Fix any failures before proceeding.
...
- Error: "Template not found: {{templates_data_mesh_contract_template}}"
