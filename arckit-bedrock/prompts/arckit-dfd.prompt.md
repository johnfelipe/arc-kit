...
Read `{{skills_mermaid_syntax_references_flowchart}}` for official Mermaid syntax — node shapes, edge labels, subgraphs, and styling options.
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **DFD** per-type checks pass. Fix any failures before proceeding.
...
- **First**, check if `{{templates_dfd_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_dfd_template}}` (default)
