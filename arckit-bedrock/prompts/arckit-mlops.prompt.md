...
- **First**, check if `{{templates_mlops_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_mlops_template}}` (default)
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **MLOPS** per-type checks pass. Fix any failures before proceeding.
