...
- **First**, check if `{{templates_servicenow_design_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_servicenow_design_template}}` (default)
...
Use the template at `{{templates_servicenow_design_template}}` as the structure.
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **SNOW** per-type checks pass. Fix any failures before proceeding.
