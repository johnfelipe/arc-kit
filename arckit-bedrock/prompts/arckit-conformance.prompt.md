...
c. `{{conformance_rules}}` in the project root (if exists):
...
- **First**, check if `{{templates_conformance_assessment_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_conformance_assessment_template}}` (default)
...
- `{{conformance_rules}}` in the project root
...
1. Read `{{conformance_rules}}` if it exists
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **CONF** per-type checks pass. Fix any failures before proceeding.
...
5. [IF custom rules missing:] Consider creating `{{conformance_rules}}` for project-specific rules
