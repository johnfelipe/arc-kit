...
Read `{{templates_gov_reuse_template}}` for the output structure.
...
Before writing, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **GOVR** per-type checks pass. Fix any failures before proceeding.
