...
Read `{{skills_wardley_mapping_references_evolution_stages}}` for supporting context on component characteristics.
...
- **First**, check if `{{templates_wardley_value_chain_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_wardley_value_chain_template}}` (default)
...
Run `bash {{scripts_bash_create_project}} --json` to get the current project path. Extract `project_id` and `project_path` from the JSON response.
...
**Before writing the file**, read `{{references_quality_checklist}}` and verify all **Common Checks** pass. Fix any failures before proceeding.
