...
- **First**, check if `{{templates_project_plan_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_project_plan_template}}` (default)
...
Read `{{skills_mermaid_syntax_references_gantt}}` and `{{skills_mermaid_syntax_references_flowchart}}` for official Mermaid syntax — date formats, task statuses, node shapes, edge labels, and styling options.
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **PLAN** per-type checks pass. Fix any failures before proceeding.
