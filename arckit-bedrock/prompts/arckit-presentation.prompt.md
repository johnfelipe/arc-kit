...
- **First**, check if `{{templates_presentation_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_presentation_template}}` (default)

> **Tip**: Users can customize templates with `/arckit:customize presentation`

### Step 4b: Load Mermaid Syntax References

Read `{{skills_mermaid_syntax_references_quadrantChart}}`, `{{skills_mermaid_syntax_references_c4}}`, `{{skills_mermaid_syntax_references_pie}}`, and `{{skills_mermaid_syntax_references_gantt}}` for official Mermaid syntax — quadrant chart axes, C4 diagram elements, pie chart syntax, date formats, and styling options.
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **PRES** per-type checks pass. Fix any failures before proceeding.
