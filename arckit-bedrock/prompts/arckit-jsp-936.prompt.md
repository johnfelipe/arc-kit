...
- **First**, check if `{{templates_jsp_936_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_jsp_936_template}}` (default)
...
Read `{{skills_mermaid_syntax_references_flowchart}}` for official Mermaid syntax — node shapes, edge labels, subgraphs, and styling options. Use this reference when generating architecture diagrams and approval pathway flowcharts in the output.
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **JSP936** per-type checks pass. Fix any failures before proceeding.
