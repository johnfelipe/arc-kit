...
- **Citation traceability**: When referencing content from external documents, follow the citation instructions in `{{references_citation_instructions}}`. Place inline citation markers (e.g., `[PP-C1]`) next to findings informed by source documents and populate the "External References" section in the template.
...
- **Doctrine assessment**: Read `{{skills_wardley_mapping_references_doctrine}}` — Score the organization's doctrine maturity (communication, development, operation, learning, leading) and identify weaknesses that affect strategic execution.
- **Gameplay patterns**: Read `{{skills_wardley_mapping_references_gameplay_patterns}}` — Identify applicable offensive patterns (tower & moat, ecosystem, open source play) and defensive patterns. Flag anti-patterns (legacy trap, premature innovation).
- **Climatic patterns**: Read `{{skills_wardley_mapping_references_climatic_patterns}}` — Assess external forces (everything evolves, co-evolution, efficiency enables innovation, inertia, technology waves) and their impact on each component.
- **Mapping examples**: Read `{{skills_wardley_mapping_references_mapping_examples}}` — Use worked examples (E-Commerce, DevOps Platform, ML Product) as positioning benchmarks for common component types.
...
- **First**, check if `{{templates_wardley_map_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_wardley_map_template}}` (default)
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **WARD** per-type checks pass. Fix any failures before proceeding.
