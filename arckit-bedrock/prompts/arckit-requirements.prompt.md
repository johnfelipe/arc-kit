...
4. **Read the template** (with user override support):
   - **First**, check if `{{templates_requirements_template}}` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `{{templates_requirements_template}}` (default)
...
9. **Write the output**:
   ...
   Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **REQ** per-type checks pass. Fix any failures before proceeding.
