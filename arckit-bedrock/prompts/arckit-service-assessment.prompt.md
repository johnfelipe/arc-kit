...
- **First**, check if `{{templates_service_assessment_prep_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_service_assessment_prep_template}}` (default)
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **SVCASS** per-type checks pass. Fix any failures before proceeding.
