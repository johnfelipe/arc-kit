...
- Read `{{templates_gcp_research_template}}` for output structure
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **GCRS** per-type checks pass. Fix any failures before proceeding.
