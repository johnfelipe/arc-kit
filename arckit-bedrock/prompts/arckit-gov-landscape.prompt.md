...
Read `{{templates_gov_landscape_template}}` for the output structure.
...
Before writing, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **GLND** per-type checks pass. Fix any failures before proceeding.
