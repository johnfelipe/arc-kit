...
Read `{{skills_mermaid_syntax_references_gantt}}`, `{{skills_mermaid_syntax_references_flowchart}}`, `{{skills_mermaid_syntax_references_pie}}`, `{{skills_mermaid_syntax_references_timeline}}`, and `{{skills_mermaid_syntax_references_mindmap}}` for official Mermaid syntax — date formats, task statuses, node shapes, edge labels, pie chart syntax, timeline formatting, mindmap indentation, and styling options.
...
- **First**, check if `{{templates_story_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_story_template}}` (default)
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **STORY** per-type checks pass. Fix any failures before proceeding.
