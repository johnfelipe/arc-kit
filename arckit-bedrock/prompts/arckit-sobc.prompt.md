...
7. **Generate comprehensive SOBC**:
   ...
   **Read the template** (with user override support):
   - **First**, check if `{{templates_sobc_template}}` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `{{templates_sobc_template}}` (default)
...
Use the template at `{{templates_sobc_template}}` as the structure. Fill in with:
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **SOBC** per-type checks pass. Fix any failures before proceeding.
