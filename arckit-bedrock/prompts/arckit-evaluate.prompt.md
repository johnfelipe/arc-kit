...
3. **Read the templates** (with user override support):
   - **First**, check if `{{templates_evaluation_criteria_template}}` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `{{templates_evaluation_criteria_template}}` (default)
   - **Also read** the scoring template: check `{{templates_vendor_scoring_template}}` first, then `{{templates_vendor_scoring_template}}`
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **EVAL** per-type checks pass. Fix any failures before proceeding.
