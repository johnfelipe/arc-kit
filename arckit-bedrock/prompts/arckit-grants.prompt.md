...
- `ARC-*-REQ-*.md` in `projects/{project}/` — Requirements specification
  ...
  - If missing: warn user to run `/arckit:requirements` first, but proceed using `{{input_architecture_docs}}` as the project description
...
1. **Read the template** (with user override support):
   - **First**, check if `{{templates_grants_template}}` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `{{templates_grants_template}}` (default)

2. Before writing, read `{{references_quality_checklist}}` and verify all **Common Checks** pass. Fix any failures before proceeding.
...
2. **If no tech note exists**: Read the tech note template at `{{templates_tech_note_template}}` and create a new file at `projects/{project-dir}/tech-notes/{grant-slug}.md`. Populate from research findings.
