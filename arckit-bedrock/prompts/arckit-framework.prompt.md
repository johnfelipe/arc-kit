...
- Check if `{{templates_custom_framework_overview_template}}` exists in the project root (user override)
- If not found: Check `{{templates_framework_overview_template}}` (user override)
- If not found: Read `{{templates_framework_overview_template}}` (default)
...
Before writing, read `{{references_quality_checklist}}` and verify all **Common Checks** plus any FWRK-specific checks pass. Fix any failures before proceeding.
