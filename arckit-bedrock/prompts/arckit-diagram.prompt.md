...
1. Read `{{skills_mermaid_syntax_references_c4_layout_science}}` for research-backed graph drawing guidance — Sugiyama algorithm, tier-based declaration ordering, edge crossing targets, C4 colour standards, and prompt antipatterns.
2. Read the type-specific Mermaid syntax reference:
   - **C4 Context / C4 Container / C4 Component**: Read `{{skills_mermaid_syntax_references_c4}}`
   - **Deployment**: Read `{{skills_mermaid_syntax_references_flowchart}}`
   - **Sequence**: Read `{{skills_mermaid_syntax_references_sequenceDiagram}}`
   - **Data Flow with ER content**: Also read `{{skills_mermaid_syntax_references_entityRelationshipDiagram}}`

**If PlantUML format selected:**

1. Read `{{skills_plantuml_syntax_references_c4_plantuml}}` for C4-PlantUML element syntax, directional relationships, layout constraints, and **layout conflict rules** (critical for preventing `Rel_Down`/`Lay_Right` contradictions).
2. For Sequence diagrams: also read `{{skills_plantuml_syntax_references_sequence_diagrams}}`
...
- **First**, check if `{{templates_architecture_diagram_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_architecture_diagram_template}}` (default)
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **DIAG** per-type checks pass. Fix any failures before proceeding.
