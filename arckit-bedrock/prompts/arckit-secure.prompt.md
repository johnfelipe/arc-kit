...
1. **Loading the template** (with user override support):
   - **First**, check if `{{templates_ukgov_secure_by_design_template}}` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `{{templates_ukgov_secure_by_design_template}}` (default)
...
15. **Save the document**:
    ...
    Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **SECD** per-type checks pass. Fix any failures before proceeding.
