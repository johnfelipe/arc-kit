...
2. **Read existing artifacts** from the project context:
   ...
   **Read the template** (with user override support):
   - **First**, check if `{{templates_dld_review_template}}` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `{{templates_dld_review_template}}` (default)
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **DLDR** per-type checks pass. Fix any failures before proceeding.
