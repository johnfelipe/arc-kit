...
- **First**, check if `{{templates_architecture_strategy_template}}` exists in the project root
- **If found**: Read the user's customized template (user override takes precedence)
- **If not found**: Read `{{templates_architecture_strategy_template}}` (default)
...
Read `{{skills_mermaid_syntax_references_flowchart}}` and `{{skills_mermaid_syntax_references_gantt}}` for official Mermaid syntax — node shapes, edge labels, date formats, task statuses, and styling options.
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **STRAT** per-type checks pass. Fix any failures before proceeding.
