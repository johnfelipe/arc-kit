...
7. **For EACH risk identified, create comprehensive risk profile**:
   ...
   **Read the template** (with user override support):
   - **First**, check if `{{templates_risk_register_template}}` exists in the project root
   - **If found**: Read the user's customized template (user override takes precedence)
   - **If not found**: Read `{{templates_risk_register_template}}` (default)
...
11. **Write the output**:
    ...
    Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **RISK** per-type checks pass. Fix any failures before proceeding.
...
Use the template at `{{templates_risk_register_template}}` as the structure. Fill in with:
