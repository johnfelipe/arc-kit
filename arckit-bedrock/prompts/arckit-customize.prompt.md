...
ArcKit uses document templates to generate consistent architecture artifacts. Users can customize these templates by copying them to `{{templates}}`. When a template exists in the custom directory, it takes precedence over the default template.

**Template locations:**

- **Defaults**: `{{templates}}` (shipped with ArcKit, refreshed by `arckit init`)
- **User overrides**: `{{templates}}` (your customizations, preserved across updates)
...
If user wants to see available templates, use Glob to find `{{templates_*_template}}` and `{{templates_*_template}}`, then extract the template name from each filename (strip the `-template.md`/`.html` suffix).
...
2. Use the Read tool to read the source template from `{{templates_{name}_template_{ext}}}`
...
4. Use the Write tool to save it to `{{templates_{name}_template_{ext}}}` (the directory will be created automatically)
5. If the source template does not exist, inform the user and suggest running `/arckit:customize list`

**Copy all templates:**

1. Use Glob to find all `{{templates_*_template}}` and `{{templates_*_template}}` files
2. For each template found, use Read to load it, update the origin banner (change `Template Origin: Official` to `Template Origin: Custom | Based On: /arckit.{command}`), and Write to save it to `{{templates}}`
...
Your template has been copied to `{{templates}}`. You can now customize it.

### How It Works

When you run an ArcKit command (e.g., `/arckit:requirements`):

1. Command checks: Does `{{templates_requirements_template}}` exist?
2. **If YES** → Uses YOUR customized template
3. **If NO** → Uses default from `{{templates}}`
...
- Default templates in `{{templates}}` are refreshed by `arckit init`
- Your customizations in `{{templates}}` are **preserved**
- Compare your templates with defaults periodically to adopt new features

To see the current default template, use the Read tool on `{{templates_{name}_template}}`.

To compare your customization with the default, read both files and compare the content.

### Reverting to Default

To stop using a custom template and revert to default, delete `{{templates_{name}_template}}`.
