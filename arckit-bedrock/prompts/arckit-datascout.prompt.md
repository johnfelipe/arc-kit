...
- Read `{{templates_datascout_template}}` for output structure
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **DSCT** per-type checks pass. Fix any failures before proceeding.
