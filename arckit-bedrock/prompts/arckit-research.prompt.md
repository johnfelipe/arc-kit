...
- Read `{{templates_research_findings_template}}` for output structure
...
Before writing the file, read `{{references_quality_checklist}}` and verify all **Common Checks** plus the **RSCH** per-type checks pass. Fix any failures before proceeding.
...
5. **If no tech note exists**: Read the tech note template at `{{templates_tech_note_template}}` and create a new file at `projects/{project-dir}/tech-notes/{topic-slug}.md`. Populate from research findings.
